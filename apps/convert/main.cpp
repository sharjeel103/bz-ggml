...
int main(int argc, char ** argv) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        else if (a == "--device") cuda_device = atoi(arg(argc, argv, i, "--device"));
    ...
    bool loaded = false;
    if (use_gpu) {
        loaded = model.load_device(model_path, cuda_device);
    } else {
        loaded = model.load(model_path, false);
    }
    if (!loaded) { fprintf(stderr, "failed to load model\n"); return 1; }
    printf("backend: %s\n", model.backend.name());
